 ...
 */
pub fn run(text_vec:&Vec<Vec<char>>, query_vec:&Vec<Vec<char>>,
  ...
  for i in 0..nh {
    ...
    assert! (res.len() == mh, "|res| should be the height of the pattern map");
    // Convert res => res_row
    ...
    for j in 0..mh {
      ...
      for k in 0..res[j].len() {
        assert! (res_row[res[j][k]] == 65535, "There must be only one pattern which matches here");
